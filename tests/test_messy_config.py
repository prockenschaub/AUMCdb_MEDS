...
from AUMCdb_MEDS import MESSY_CFG, PIPELINE_NAME

EXPECTED_TABLES = {
    "patient": {"dob", "gender", "dod"},
    "admissions": {"icu_admission", "icu_discharge", "weight", "height"},
    "numericitems": {"event"},
    "listitems": {"event"},
    "freetextitems": {"event"},
    "procedureorderitems": {"event"},
    "processitems": {"start", "end"},
    "drugitems": {"start", "rate", "dose", "end"},
}
...
def test_expected_tables_and_events(cfg: MessyConfig):
    ...
    assert by_prefix == EXPECTED_TABLES
...
def test_value_columns_are_column_reads_not_literals(cfg: MessyConfig):
    """`numeric_value`/`text_value` read columns, not bare-string literals.

    The 0.6.x config wrote `numeric_value: value`, which under dftly would be the *literal*
    string "value" rather than a read of the `value` column -- a silent data-corruption trap
    rather than a load error, so it is worth pinning.
    """
    by_prefix = {t.input_prefix: t for t in cfg.event_tables}

    numeric = by_prefix["numericitems"].events[0]
    assert "value" in numeric.referenced_columns
...
def test_etl_block(cfg: MessyConfig):
    ...
    assert cfg.etl.dataset_name == "AUMCdb"
