#!/usr/bin/env python
"""End-to-end driver for the AmsterdamUMCdb MEDS ETL.
...
which is the one part of the ETL that is not yet expressible in MESSY (unzipping the DataVerse
archives and deriving the patient table's pseudo-timestamps from admission offsets).

If you do not need the pre-MEDS step re-run, skip this wrapper entirely:

    meds-extract-run spec=AUMCdb output_dir=$OUTPUT_DIR
"""
